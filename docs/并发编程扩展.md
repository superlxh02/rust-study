...
Rust的 `Mutex`不是递归锁，同一线程重复 `lock`会导致死锁：
...
let _g2 = lock.lock().unwrap(); // 死锁！因为当前线程已经持有锁
```

> **原理**：Rust的 `Mutex`不计数持有者，再次请求会阻塞自己。
...
虽然不是标准库，但值得提及：`parking_lot`库提供了更安全的 `Mutex`和 `RwLock`，支持死锁检测、避免递归死锁等。标准库用户可以借助外部工具。
