...
当你只关心匹配某一种模式而不想写完整的 `match` 时，`if let` 提供更简洁的写法。
...
`as` 运算符用于在 Rust 中进行**显式类型转换**（又称“类型铸造”）。它可以将一种原始类型转换为另一种原始类型（如整数、浮点数、字符），或者将引用转换为更具体的类型。`as` 不会改变内存中的位模式（除非是整数到整数的转换），通常用于数值类型之间的安全转换。
...
fn main() {
    ...
    *ref_mut += 1;          // 通过可变引用修改原值
    ...
    // 下面的代码会编译错误（取消注释查看）：
    // let ref3 = &data;    // 不可变借用，但可变借用 ref_mut 还在作用域内
